...
class _Base(object):
    ...
    def _update_tasks(self, files):
        playbooks = []
        reg1 = re.compile(self.regex1)
        reg2 = re.compile(self.regex2)
        playbooks.extend(filter(reg1.match, files))
        playbooks.extend(filter(reg2.match, files))
