...
class Project(AbstractModel):
    ...
    def _prepare_kw_api(self, kind, **extra):
        """
        
        :param kind: moduleapi/playbookapi
        :param extra: {
        ...
        from .tasks import History
        history_kwargs = dict(
            mode=extra["data"]["module"], start_time=timezone.now(),
            inventory=extra["inventory"], project=self,
            kind=kind, raw_stdout="", execute_args={"args": extra["data"]["args"]},
            initiator=extra["user"].id,
        )
        if isinstance(extra["inventory"], (six.string_types, six.text_type)):
            history_kwargs['inventory'] = None

        history = History.objects.create(status="DELAY", **history_kwargs)
        # tra ra kwargs
        kwargs = dict(
            data=extra["data"], inventory=extra["inventory"], history=history, project=self
        )
        return kwargs

    #excute job from ansible client API
    def execute_api(self, kind, **extra):
        """
        ...
        kind = kind.upper()
        #goi class trong setting
        task_class = self.task_handlers.backend(kind)

        sync = extra.pop("sync", False)


        kwargs = self._prepare_kw_api(kind, **extra)
        ...
        if sync:
            ...
            task_class.delay(**kwargs)
        return kwargs['history'].id if kwargs['history'] is not None else kwargs['history']



        # excute job from ansible client API

    def execute_api_testing(self, kind, **extra):
        """
