...
# Author: dongvt
"""
File này mục đích để làm phần ansible client API thay vì dùng execute command.
...
from ansible.module_utils.six import string_types, text_type, iteritems
...
from polemarch.main.models import Host
...
# Phần này sẽ overwrite lại các class cần thiết để tích hợp ansible client api vào tool



class AnsibleResultsCallback(CallbackBase):

    """
    Plugin call back của result sau khi chạy ansible trả về ,
    phần này có nhiều module và format có sẵn như slack..
    """
    def __init__(self, *args, **kwargs):
        super(AnsibleResultsCallback, self).__init__(*args, **kwargs)
        self.return_data = []
    ...
    def v2_runner_on_unreachable(self, result, ignore_errors=False):
        ...
        # return data
        self.return_data.append(self._command_generic_msg(host=name, task=task, result=result._result, changed=result._result['changed'], status="unreachable"))

    def v2_runner_on_ok(self, result, *args, **kwargs):
        name = result._host.get_name()
        task = result._task.get_name()
        if task == "setup":
            self.return_data.append(self._command_generic_msg(host=name, task=task, result=result._result, changed=result._result['changed'], status="ok"))
        else:
            self.return_data.append(self._command_generic_msg(host=name, task=task, result=result._result, changed=result._result['changed'], status="ok"))
    ...
    def v2_runner_on_failed(self, result, *args, **kwargs):
        name = result._host.get_name()
        task = result._task.get_name()
        self.return_data.append(self._command_generic_msg(host=name, task=task, result=result._result, changed=result._result['changed'], status="failed"))

    def v2_runner_on_skipped(self, result):
        name = result._host.get_name()
        task = result._task.get_name()
        self.return_data.append(self._command_generic_msg(host=name, task=task, result=result._result, changed=result._result['changed'], status="skipped"))
        ...
        if len(source) > 0:
            ...
            for inven in source:
                ...
                for host_ungroup in inven.hosts_list:
                    self._inventory.add_host(host=host_ungroup.name, group="ungrouped")
...
class AnsibleOption(object):
    def __init__(self, module_path=None, forks=None, become=False, listhosts=False, listtasks=False, listtags=None,
                 verbosity=None, subset=None, extra_vars=None, ask_vault_pass=None,
                 vault_password_files=None, new_vault_password_file=None, output_file=None, one_line=None, tree=None, ask_sudo_pass=None, ask_su_pass=None, sudo=None,
                 ...
                 remote_user='root', connection='smart', timeout=300):
