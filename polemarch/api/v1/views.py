...
from polemarch.main import models, utils
...
class ProjectViewSet(base.PermissionMixin, base.ModelViewSetSet,
    ...
    POST_WHITE_LIST = ['sync', 'execute_playbook', 'execute_module', 'execute_script', 'execute_module_api']
    ...
    @detail_route(methods=["post"], url_path="execute-script")
    def execute_script(self, request, *args, **kwargs):
        serializer = self.get_serializer(self.get_object())
        return serializer.execute_script(request).resp

    # new feature author: dongvt
    # chạy api ansible module
    @detail_route(methods=["post"], url_path="execute-module-api")
    def execute_module_api(self, request, *args, **kwargs):
        serializer = self.get_serializer(self.get_object())
        return serializer.execute_module_api(request).resp
...
class StatisticViewSet(base.ListNonModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        stats = OrderedDict(
            projects=self._get_count_by_user(models.Project),
            templates=self._get_count_by_user(models.Template),
            inventories=self._get_count_by_user(models.Inventory),
            groups=self._get_count_by_user(models.Group),
            hosts=self._get_count_by_user(models.Host),
            teams=self._get_count_by_user(models.UserGroup),
            users=self._get_count_by_user(User),
        )
        stats['jobs'] = self._get_history_stats(request)
        return base.Response(stats, 200).resp

class ScriptViewSet(base.PermissionMixin, base.ModelViewSetSet, _GroupedViewSet):
    model = models.Script
    serializer_class = serializers.ScriptSerializer
    serializer_class_one = serializers.OneScriptSerializer
