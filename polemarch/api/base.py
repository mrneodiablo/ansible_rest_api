...
class Response(_ResponseClass):
    """
    Tạo class Respone giúp over write respone format của api
    OrderedDict([('data', ['GIT', 'MANUAL', 'TAR']), ('status', 200)])
    ...
    # trả về respone JSON String
    @property
    def resp(self):
        return RestResponse(**self._asdict())

    # Trả về respone là đối tượng DICT
    @property
    def resp_dict(self):
        return self._asdict()
...
class QuerySetMixin(rest_views.APIView):
    """
    request --> methos get, post, put
    
    
    Là lớp Base class cho tất cả cả QueryMix modle
    Lớp cha APIView xửu lý các request và gởi nó tới các handler method như get(), post, delete ứng với method của request
...
class GenericViewSet(QuerySetMixin, viewsets.GenericViewSet):
    """
    Lớp GenericViewSet cung cấp các phương thức như update delete list create .., sau đó dùng route để map với menthod cua request
...
"""
Using mixins
cung cấp các tính nằng update update retrieve list create .. từ request --> serializer --> model
"""
